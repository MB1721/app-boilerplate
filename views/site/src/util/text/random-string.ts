export default function randomString(length: number) {
  ...
  for (let strIdx = 1; strIdx <= length; strIdx++) {
    ...
    for (let chBit = 1; chBit <= 2; chBit++) {
      const maxDigit: number = chBit === 1 ? 7 : 14; // first digit max is 7, second is e
      const minDigit: number = chBit === 1 ? 2 : 1; // first digit min is 2, second is 1
      
      let randomHexDigit: string | number = Math.floor(Math.random() * (maxDigit + 1)); // int from 0 - 14 (word characters)
    ...
    str += String.fromCharCode(Number(hexCharArr)); // add char to string
  }
  return str;
}